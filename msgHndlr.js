...
module.exports = msgHandler = async (CR4R, message) => {
    try {
        ...
        function kotor(ktanya){a = ['jembut','anjing','jing','anjng','bjngan','bjngn','bjgn','bajingan','goblok','gblok','gblk','gila','tolol','asw','asu','kontol','kontl','kntl','bngke','bangke','tlol','anjng','anjing','jing','njir','anjir','wanjir','memek','mek'];try{b = ktanya.split(' ')}catch(err){b = ktanya};for(i=0,len=b.length; i<len; i++){if(a.indexOf(b[i]) >= 0) return 'ok'}}
        ...
        case 'akunnord':
            ...
            axios.get('https://raw.githubusercontent.com/cr4r/ceknord/main/akun').then(resp => { 
                ...
                CR4R.reply(from,`Speed: ${processTime(t, moment())} _Detik_\n\n${dataanya[rndm(dataanya.length)]}`,id);
            })
            break
        case 'save':
        ...
        case 'undi':
            ...
            if(!isGroupAdmins) return CR4R.reply(from,'Perintah ini hanya untuk admin ya, jadi hubungi admin untuk memulai undian',id)
            const memberNya = await CR4R.getGroupMembers(groupId)
            let hayiu = 'Speed: ${processTime(t, moment())} _Detik_\n\n'
        ...
        case 'sgif':
            ...
            if(cek()==='ok') return CR4R.reply(from,maintan,id)
            if (!isBlocked) return CR4R.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot',id)
            if (args.length !== 1) return CR4R.reply(from, `Maaf, format pesan salah.\nKetik pesan dengan sgif <link_giphy>`, id)
            const isGiphy = url.match(new RegExp(/https?:\/\/(www\.)?giphy.com/, 'gi'))
            const isMediaGiphy = url.match(new RegExp(/https?:\/\/media.giphy.com\/media/, 'gi'))
            if (isGiphy) {
                const getGiphyCode = url.match(new RegExp(/(\/|\-)(?:.(?!(\/|\-)))+$/, 'gi'))
                if (!getGiphyCode) { return CR4R.reply(from, 'Gagal mengambil kode giphy', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                CR4R.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    CR4R.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                }).catch((err) => console.log(err))
            } else if (isMediaGiphy) {
                const gifUrl = url.match(new RegExp(/(giphy|source).(gif|mp4)/, 'gi'))
                if (!gifUrl) { return CR4R.reply(from, 'Gagal mengambil kode giphy', id) }
                const smallGifUrl = url.replace(gifUrl[0], 'giphy-downsized.gif')
                CR4R.sendGiphyAsSticker(from, smallGifUrl)
                .then(() => {
                    CR4R.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                })
                .catch(() => {
                    CR4R.reply(from, `Ada yang error!`, id)
                })
            } else {
                await CR4R.reply(from, 'Maaf, command sticker giphy hanya bisa menggunakan link dari giphy.  [Giphy Only]', id)
            }
            break
        case 'donasi':
        case 'donate':
        ...
        case 'ss':
            ...
            await ss(_query).then((result) => {
                CR4R.sendFile(from, `./log/screenshot.jpeg`, 'ss.jpeg', `${donasi}`, id)
